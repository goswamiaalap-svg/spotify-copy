...
const regex = /function NowPlayingScreen\(\{ onClose \}\) \{[\s\S]*?\n\/\/\s*─────────────────────────────────────\r?\n\/\/\s*LIBRARY PAGE \(mobile\)/;
...
const replacement = `function NowPlayingScreen({ onClose }) {
  ...
  const fetchVideo = async () => {
    ...
    const searchId = \`search:\${encodeURIComponent(q)}\`;
    setVideoId(searchId);
    setShowVideo(true);
    setVideoLoading(false);
  };
            ...
            <div style={{position:'relative', marginBottom:'20px', padding:'0 4px'}}>
              {showVideo && videoId ? (
                <div style={{width:'100%', aspectRatio:'16/9', borderRadius:'12px', overflow:'hidden', background:'#000', position:'relative'}}>
                  <iframe
                    src={videoId?.startsWith('search:')
                      ? \`https://www.youtube.com/results?search_query=\${videoId.replace('search:','')}\`
                      : \`https://www.youtube-nocookie.com/embed/\${videoId}?autoplay=1&controls=1&rel=0\`
                    }
                  ...
                  <button onClick={() => setShowVideo(false)}
                    style={{position:'absolute', top:'10px', right:'10px', background:'rgba(0,0,0,0.8)', backdropFilter:'blur(8px)', border:'none', borderRadius:'500px', color:'white', padding:'8px 14px', fontSize:'12px', fontWeight:700, cursor:'pointer', zIndex:10}}>
                    ← Artwork
                  </button>
                </div>
              ) : (
                <div style={{position:'relative'}}>
                  ...
                  {/* Switch to Video button — always visible */}
                  <button
                    onClick={fetchVideo}
                    ...
                    {videoLoading ? (
                      ...
                      <>
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="white"><path d="M17 10.5V7c0-.55-.45-1-1-1H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.55 0 1-.45 1-1v-3.5l4 4v-11l-4 4z"/></svg>
                        Switch to video
                      </>
                    )}
                  </button>
...
console.log('REPLACED_SUCCESSFULLY_REGEX');
